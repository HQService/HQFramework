...
class PlayerSkullRepository(
    ...
    private val skinProfileMap = mutableMapOf<UUID, com.destroystokyo.paper.profile.PlayerProfile>()
        ...
        if (skinProfileMap.containsKey(targetUniqueId)) {
            if (!itemStack.type.isAir && targetedItemStack.isSimilar(itemStack)) {
                ...
                if (skullMeta is SkullMeta) {
                    skullMeta.playerProfile = skinProfileMap[targetUniqueId]
            ...
            if (lambdaQueueMap.containsKey(targetUniqueId)) lambdaQueueMap[targetUniqueId]?.offer(itemStack)
                ...
                coroutineScope.launch(Dispatchers.BukkitAsync) {
                    ...
                    val profile = Bukkit.createProfile(offlinePlayer.uniqueId)
                    /*val url = skinURLRepository.findById(offlinePlayer.uniqueId)
                    if (url != null) {
                        profile.textures.skin = URL("http://textures.minecraft.net$url")
                        skinProfileMap[targetUniqueId] = profile
                    } else {
                        val url = offlinePlayer.playerProfile.textures.skin?.path
                        if (url != null) {
                            skinURLRepository.upload(offlinePlayer.uniqueId, url)
                            profile.textures.skin = URL("http://textures.minecraft.net$url")
                        }
                    }*/
                    skinProfileMap[targetUniqueId] = profile
