...
class NettyChannelContainer(
    ...
    fun onChannelActive(port: Int, wrapper: ChannelWrapper) {
        val name = ProxyServer.getInstance().servers.values.firstOrNull { it.address.port == port }?.name?: "Unknown-${unknownClientId++}"
        ProxyServer.getInstance().pluginManager.callEvent(NettyClientConnectedEvent(wrapper, wrapper.handler.connectionState, name))
    ...
    private fun onChannelInactive(wrapper: ChannelWrapper) {
        ProxyServer.getInstance().pluginManager.callEvent(NettyClientDisconnectedEvent(wrapper, wrapper.handler.connectionState))
