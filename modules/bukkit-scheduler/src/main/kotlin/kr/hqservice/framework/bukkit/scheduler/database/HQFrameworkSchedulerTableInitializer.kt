...
class HQFrameworkSchedulerTableInitializer(private val dataSource: DataSource, private val logger: Logger) {
    @Suppress("SqlDialectInspection", "SqlNoDataSourceInspection", "DuplicatedCode")
    private companion object DDL {
        const val SHOW_TABLES = "SHOW TABLES LIKE 'hqframework_quartz%'"
        const val SHOW_TABLES_SQLITE =
            "SELECT name FROM sqlite_master WHERE type ='table' AND name LIKE 'hqframework_quartz%'"
        const val INITIALIZE_TABLES = "DROP TABLE IF EXISTS hqframework_quartz_FIRED_TRIGGERS;\n" +
        ...
        const val INITIALIZE_TABLES_INNODB =
    ...
    @Setup
    fun setup() {
        val tableListQuery = when (dataSource) {
            is SQLiteDataSource -> {
                SHOW_TABLES_SQLITE
            }
        ...
        val tablesSize = dataSource.connection.use { connection ->
            connection.prepareStatement(tableListQuery, ResultSet.TYPE_FORWARD_ONLY).use { preparedStatement ->
    ...
    private fun dropIfExistsAndCreateTables() {
        val initializeQuery = when (dataSource) {
            is MySQLDataSource -> INITIALIZE_TABLES_INNODB
            is SQLiteDataSource -> INITIALIZE_TABLES
            else -> throw UnsupportedOperationException("unsupported datasource provided while initializing tables.")
        }
        logger.info("${AnsiColor.CYAN}Initializing Scheduler tables...")
        dataSource.connection.use { connection ->
            connection.autoCommit = false
            connection.prepareStatement(initializeQuery).use { preparedStatement ->
