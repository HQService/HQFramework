...
class CallbackContainer {
    ...
    fun addOnQueue(channel: ChannelWrapper, packet: Packet, targetClass: KClass<out Packet>, callback: PacketCallbackHandler<out Packet>) {
        try {
            lock.writeLock().lock()
    ...
    fun complete(packet: Packet): Boolean {
        try {
            ...
            return if (!queue.isNullOrEmpty()) {
                val callback : PacketCallbackHandler<Packet> = queue.poll() as? PacketCallbackHandler<Packet> ?: return false
