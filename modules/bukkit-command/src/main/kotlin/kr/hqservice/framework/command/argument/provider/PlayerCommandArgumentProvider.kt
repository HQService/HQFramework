...
@Component
class PlayerCommandArgumentProvider(
    private val server: Server
) : CommandArgumentProvider<Player> {
    override suspend fun cast(context: CommandContext, argument: String?): Player {
        argument ?: throw ArgumentFeedback.RequireArgument
        return server.onlinePlayers.firstOrNull {
            ChatColor.stripColor(it.displayName) == argument || it.name == argument
        } ?: throw ArgumentFeedback.PlayerNotFound
    }
