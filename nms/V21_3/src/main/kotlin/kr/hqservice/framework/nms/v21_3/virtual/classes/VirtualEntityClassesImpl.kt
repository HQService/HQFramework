package kr.hqservice.framework.nms.v21_3.virtual.classes
...
class VirtualEntityClassesImpl(
    ...
    override val entitySpawnPacket: VirtualMessageConstructor = VirtualMessageConstructor {
        val npcServerEntity = ServerEntity(
            (it.getEntity() as Entity).level() as ServerLevel, (it.getEntity() as Entity), 0, false,
            { _: Packet<*>? -> }, setOf<ServerPlayerConnection>()
        )
    ...
    override val entityTeleportPacket: VirtualMessageConstructor = VirtualMessageConstructor {
        val entity = it.getEntity() as Entity
        ClientboundTeleportEntityPacket(entity.id, PositionMoveRotation.of(entity), setOf(), entity.onGround)
    }
